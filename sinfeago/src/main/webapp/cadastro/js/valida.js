...
function tel_mask(v) {
	...
	v = v.replace(/(\d{0})(\d)/, "$1($2") // Coloca ( antes do primeiro
	// d�gitos
	v = v.replace(/(\d{2})(\d)/, "$1) $2") // Coloca ) entre o segundo e o
	// terceiro d�gitos
	v = v.replace(/(\d{4})(\d)/, "$1-$2") // Coloca - entre o sexto e o s�timo
	// d�gitos
	return v
}
...
function cep_mask(v) {
	...
	// terceiro d�gitos
	v = v.replace(/(\d{3})(\d)/, "$1-$2") // Coloca tra�o entre o quinto e o
	// sexto d�gitos
	return v
}
...
function data_mask(v) {
	...
	v = v.replace(/(\d{2})(\d)/, "$1/$2") // Coloca / entre o segundo e o
	// terceiro d�gitos
	v = v.replace(/(\d{2})(\d)/, "$1/$2") // Coloca / entre o quarto e o
	// quinto d�gitos
	return v
}
